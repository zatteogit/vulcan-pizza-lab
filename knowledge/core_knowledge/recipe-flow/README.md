...
- **Allineamento Linea di Base per i Time Slots**: In `user-needs.tsx`, la label della card oraria (`TimeSlotPicker`) possiede un'altezza minima di `2.4em` per allineare perfettamente i sottotitoli orari sulla stessa linea di base su tutte le card, sia che il titolo occupi una riga sia due (es. stasera vs domani a pranzo).
- **Adattamento del Titolo a Container Query (CQW)**: In `explore.tsx` e `recommended-styles.tsx`, i titoli delle card stile usano container queries (`containerType: "inline-size"`) e la dimensione base del font scalata in `cqw` (`min(..., 10.5cqw)` o `12cqw`) con sillabazione automatica, per scalare fluidamente in base alle dimensioni reali della card nelle colonne strette (es. griglie responsive a 4 colonne) senza clippare o spezzare orribilmente i nomi lunghi (es. Contemporanea, Friarielli).
- **Rimozione Ridondanza Summary nel Pannello Parametri**: In `recipe-setup-panel.tsx`, la riga riassuntiva del trigger è stata semplificata rimuovendo i dati ripetuti (idratazione, W farina, ore e temperatura) che sono già visibili nelle stat card adiacenti, mantenendo solo il preset dello stile e l'interpretazione attiva.
- **Supporto Safe Area in RecipeView, SectionTabs e Home**: In `recipe-view.tsx` e `recipe-section-tabs.tsx`, l'header della ricetta, il contenitore del pulsante indietro flottante e la nav bar inferiore mobile applicano `env(safe-area-inset-top)` e `env(safe-area-inset-bottom)` per integrarsi con notch e barre di sistema. In `home.tsx`, la barra di notifica superiore adotta `pr-16` e `flex-wrap` per non scontrarsi con il `ProfileButton` della shell su schermi stretti.
