...
- **Pulsante e Rationale di Ottimizzazione**: Se abilitato e non già al soffitto, mostra il pulsante "Ottimizza per me" (il guadagno numerico non è visualizzato sul bottone ma come animazione "+Δ" che sale e scompare accanto al punteggio). In calce alla card viene renderizzato l'elenco dei passaggi e delle scelte prese dall'ottimizzatore (rationale).
- **Assi Extra e Formula del Punteggio in NerdMode**: Quando la modalità Nerd è attiva, `RecipeMatchCard` mostra anche gli assi secondari pesati (`sustainability`, `experimentation`) e la formula matematica esplicita con cui il composite score viene calcolato tramite la media pesata degli assi (usando gli stessi pesi CMS configurati per la ricetta).
- **Spiegazioni Assistite (Learn-Inline)**: Ogni asse nella `RecipeMatchCard` è interattivo: facendovi click sopra, spunta inline una spiegazione dettagliata (es. il ruolo della maturazione nella digeribilità) estratta direttamente dal CMS (`cms.scoreDimensions[key].explain`).
- **Stati Tono e Icone Match**: Il tono del match mappa il punteggio su 5 icone e colori diversi: `HeartHandshake` ($\ge 90$), `Heart` ($75-89$), `HeartPulse` ($60-74$), `HeartCrack` ($< 60$ o low) e `HeartOff` per fallimento. Tutti i copy associati ai toni di match e i relativi messaggi relazionali per modalità canonica o adattata sono configurati e localizzati tramite CMS (`cms.cooking.matchTones`).
