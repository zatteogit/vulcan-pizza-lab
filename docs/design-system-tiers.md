...
- **F2-3 ✅ `ds/Heading` prima passata (build verde)** — applica `.type-heading-xl/lg/md/sm` + `.type-display` oltre a `.type-title-page`.
  ...
  - ✅ **Decisione bidirezionale presa (line-height composite heading): NON allineare.** Verifica empirica (preview, 705px e 375px mobile, su home/explore/learn/profile/pre-ferments): **nessun** heading che usa `type-heading-md/sm/lg/xs` va a capo — sono tutti su riga singola. Quindi l'effetto del line-height sull'interlinea è nullo (niente da migliorare), mentre cambiarlo sposterebbe l'altezza-box anche su riga singola → ritmo verticale alterato su ogni schermata, per zero beneficio. Le composite restano com'erano. Il "mismatch" snug vs compact è quindi un non-problema in pratica.
